...
environments](/deployment/architecture/topology)). There is no per-deployment counter to
...
[second deployment](/deployment/architecture/topology#per-deployment-isolation) — which is the
...
[Self-hosted deployment](/deployment/install/kubernetes) and
[Single-VM deployment](/deployment/install/single-vm).
