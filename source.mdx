...
<Callout type="info" title="Network and security prerequisites">
  Allow outbound communication to Popsink endpoints (443/TCP), authorize your deployment IP at the database layer, and supply limited-privilege credentials (read + CDC). Review [Kubernetes requirements](/deployment/install/requirements), [Egress IP addresses](/deployment/connectivity/egress-ips) and [Tunnels and private connectivity](/deployment/connectivity/tunnels) for the detailed network matrices.
</Callout>
