...
description: "Reaching a source that is not on the data plane's network — IPsec VPN, SSH tunnel, and overlay networks such as Tailscale."
...
network at all; see
[Control plane and data plane](/deployment/architecture/control-data-plane) for why.
...
An overlay sidesteps the network team entirely: both ends dial *out* to a
coordination service and the source becomes privately routable on a virtual
network. No inbound firewall rule, no gateway appliance, no static IP, and the
source is **never published on the public internet**.

That makes it the practical answer when the source sits in a network you cannot
re-architect — a customer's data centre, an acquisition still on its own
infrastructure, anywhere an inbound rule would take a quarter to approve.

Tailscale is a WireGuard mesh, and the usable pattern is a **subnet router**:
one host inside the source network advertises the database subnet to the
tailnet, and the Popsink cluster joins the same tailnet and accepts those
routes.
...
Note that the last pattern needs **no inbound rule at all** on either side —
...
On a Popsink-managed deployment there is no tunnel to build for a source that is
already publicly reachable: allowlist our fixed per-region address instead, from
[Egress IP addresses](/deployment/connectivity/egress-ips).
...
| Everything dies at once, no config change | Tailscale key expiry, or the subnet router in the source network having stopped |
