...
| `kotatsu.s3.bucket must be provided when kotatsu is enabled`       | Kotatsu enabled with a non-`s3://` storage engine — set `kotatsu.enabled: false`                  |
...
| Broker pods refuse the storage URL as unsupported                  | A `postgres://` engine, from the wizard's retention step — the broker is object-store only, use `s3://` or `abfss://` |
| Broker pods on `abfss://` fail with `403 AuthorizationFailure`     | The Azure identity lacks `Storage Blob Data Contributor`. `Owner` is an ARM role and grants no data-plane access |
| Broker pods on `abfss://` time out or are refused, account resolves | A private endpoint was provisioned for the `dfs` sub-resource only — the broker speaks `blob`     |
