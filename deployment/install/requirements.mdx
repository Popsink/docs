...
Retention can also run on **ADLS Gen2** or on **PostgreSQL** instead of S3.
PostgreSQL is simpler to operate but does not scale the same way; ADLS Gen2 is
supported and experimental — deliberately so, since there is no continuous run
against a real account. Both rule out the optional stream browser: one has no
bucket to browse, the other no ADLS client. The ADLS Gen2 setup is on the
