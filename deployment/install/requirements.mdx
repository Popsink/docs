...
| -------------------- | ------------------------------- | ----------------------------------------------------------------------- |
| S3-compatible bucket | **Required** (or ADLS Gen2)     | Broker retention. AWS S3, MinIO or equivalent; enable object versioning  |
...
An object store is not optional: the broker keeps **no** durable state anywhere
else, and there is no PostgreSQL retention — that backend was removed from
Kodansu and a `postgres://` engine is now refused. The alternative to S3 is
**ADLS Gen2**, supported and experimental — deliberately so, since there is no
continuous run against a real account. It rules out the optional stream browser,
which has no ADLS client. Setup is on the
