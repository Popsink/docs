...
The **Retention** step only offers S3 and PostgreSQL, but the broker also reads
and writes **ADLS Gen2** — `abfss://`, `abfs://` and `az://` — from Kodansu
`1.0.0-alpha.17` onwards — the broker tag the chart pins by default is already
past it, so there is no image bump to do. What is missing is the chart: there is
no `kodansu.storage.azure` block, so this is a hand-edit of the generated
