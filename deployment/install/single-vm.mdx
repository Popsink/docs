...
> This deployment model is intentionally simple. For high-availability or large-scale production use, see the [Kubernetes deployment](/deployment/install/kubernetes) instead.
...
| Service | Requirement |
|---|---|
| **Database (PostgreSQL)** | A database instance accessible from the VM. The VM will not start if unreachable. |
...
|---|---|---|---|
| Your database instance (PostgreSQL) | Your DB port (default 5432) | **Hard** | VM fails to start |
...
|---|---|---|---|
| `DB_BACKEND` | Yes | — | Database backend: `postgres`. No default — must be set. |
...
| Parameter | Required | Default | Description |
|---|---|---|---|
| `KORA_DB_BACKEND` | Yes | — | Database backend: `postgres` or. No default — must be set. |
| `KORA_DB_HOST` | Yes | — | Kora database host (FQDN or IP) |
| `KORA_DB_PORT` | No | `5432` | Port |
| `KORA_DB_DATABASE` | Yes | — | Database name. |
...
| `popsink-configure.service` shows a database error | `DB_HOST` unreachable, wrong port, or bad credentials | Test connectivity from the VM console: `nc -zv <DB_HOST> <DB_PORT>`; for PostgreSQL verify `pg_hba.conf` allows the VM's IP; |
...
The VM is stateless — all data lives in your external database and S3 bucket, so
updating Popsink means booting a replacement VM from a newer image and switching
routing to it. There is no in-place upgrade. The step-by-step procedure is in
[Upgrades](/deployment/operate/upgrades#single-vm).
...
For production-grade scalability and high availability, consider the [Kubernetes deployment](/deployment/install/kubernetes).
