...
sidebarTitle: "Control and data plane"
---
...
either. See [Upgrading a fleet](/deployment/architecture/topology#upgrading-a-fleet).
...
Everything above is initiated **outbound, by the data plane**. That gives three
rules, and they are the whole story:

- **allow egress** from the cluster to `controlPlaneUrl` over HTTPS — it carries
  the heartbeat, config sync, metrics and login verification;
- **allow it namespace-wide**, not just for the API pod: connector workers
  refresh their own OAuth tokens;
- **open nothing inbound.** The control plane never initiates a connection, so
  there is no rule to write in that direction and no address to allow-list.

The one value that is not a firewall rule but behaves like one is `ingressUrl`:
it must match the public URL your users actually reach, and DNS must resolve it
to your ingress controller, because it is what login redirects and OAuth
callbacks return to.

The full checklist, including image-registry egress, is in
[Kubernetes requirements](/deployment/install/requirements#network). If the deployment
stays `offline`, the cause is almost always blocked egress plus a wrong
`deploymentId` / `deploymentJwtToken` — the symptom-to-cause table is on the
[troubleshooting page](/deployment/operate/troubleshooting).
...
<CardGroup cols={2}>
  ...
  <Card title="Install a deployment" icon="cloud-arrow-up" href="/deployment/install/kubernetes">
