...
deployments from a single organization, with one user and billing view. What
travels between the two is documented in
[Control plane and data plane](/deployment/architecture/control-data-plane).

How large each of those boxes gets before the design has to be split —
tables per connector, subscriptions per target, connectors per deployment — is
covered in [Sizing and limits](/limits).
...
Deployments upgrade independently — there is no fleet-wide rollout. Roll a
staging deployment first, confirm it comes back **Live**, then move through the
production ones. The procedure per form factor is in
[Upgrades](/deployment/operate/upgrades).

<CardGroup cols={2}>
  <Card title="Install a deployment" icon="cloud-arrow-up" href="/deployment/install/kubernetes">
    From the control-plane wizard to a Live deployment.
  </Card>
