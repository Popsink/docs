...
[Helm chart reference](/deployment/partners). This page covers the path that
...
[chart reference](/deployment/partners).

## Before you start

Run through [Kubernetes requirements](/deployment/installation) first. In short:
...
fleet, read [Deployments and environments](/deployment/topology) before creating
the first one.
</Note>
...
production. See
[Managing secrets](/deployment/partners#managing-secrets-with-existingsecret).
</Tip>
...
<Tabs>
  ...
  <Tab title="Let the chart render it">
    ...
    [Ingress values](/deployment/partners#ingress).
  </Tab>
</Tabs>
...
<Card
  ...
  href="/deployment/troubleshooting#the-deployment-stays-awaiting-connections"
>
...
<AccordionGroup>
  <Accordion title="Versions & images">
    ...
    - Upgrade one deployment at a time, starting with staging. See
      [Upgrading a fleet](/deployment/topology#upgrading-a-fleet).
  </Accordion>
...
2. Generate the four secrets yourself — see
   [Secrets you must generate](/deployment/partners#secrets-you-must-generate).
3. Write the `values.yaml` from the
   [Helm chart reference](/deployment/partners), which documents every component
...
<CardGroup cols={2}>
  ...
  <Card title="Troubleshooting" icon="triangle-exclamation" href="/deployment/troubleshooting">
  ...
  <Card title="Control plane and data plane" icon="shuffle" href="/deployment/control-data-plane">
