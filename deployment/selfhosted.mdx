...
- **Deployment name** – Choose a recognizable name for your environment.
- **Deployment URL** – Automatically generated from your deployment name.
- **Deployment method** – Select **Self-hosted**.
...
> ⚠️ The chart does **not** manage ingress configuration. You must configure it separately.
...
<Warning>
Use the command **the wizard shows you**, not a copy of one. It carries the chart
version this control plane supports, and `-f values.yaml` — Helm resolves values
at install time, so without it the command installs pure chart defaults and
ignores everything the wizard generated.
</Warning>
...
<Warning>
Two values are **not** generated for you and the install cannot start without
them: `imagePullSecret.token` (the registry token Popsink issued you) and
`kora.database.*` (the schema registry's own PostgreSQL role and database, which
you have to create). See
[Install Popsink with Helm for partners](https://docs.popsink.com/deployment/partners#schema-registry-database-kora).
</Warning>
