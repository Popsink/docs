...
| | |
|---|---|
| **Teradata Vantage** | 17.x and later |
| **Protocol** | Teradata SQL over port `1025` with TLS enabled |
| **Deployments** | VantageCloud and on-premises |
...
- **Infrastructure:** network connectivity between Popsink and the Teradata port (`1025` by default) with TLS encryption enabled. See [Compatible versions](#compatible-versions) for the supported Vantage releases.
