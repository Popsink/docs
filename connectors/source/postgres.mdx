...
| **Logical decoding plugin** | `pgoutput` — built into PostgreSQL, nothing to install |
| **Managed services** | Amazon RDS & Aurora PostgreSQL, Google Cloud SQL, Azure Database for PostgreSQL |
| **CDC engine** | Debezium 3.5 |

<Note>PostgreSQL 13 and earlier are outside the tested matrix and are not supported. On managed services, use the provider's own replication role rather than a superuser.</Note>
