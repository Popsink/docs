...
| **Required feature** | Table change history (`CHANGES()`) on GoogleSQL standard tables |
| **Table types** | Standard tables only — views, materialized views, and external tables are not supported |
| **API** | BigQuery API v2 |

<Note>Change history must be enabled per table (`enable_change_history = TRUE`) and is retained for up to 7 days by default.</Note>
